...
aggregate(yield ~ Variety, data = Oats, FUN = mean) |>
  knitr::kable(digits = 1, col.names = c("Variety", "Mean Yield (bu/acre)"))
```

As shown in @tbl-summary, Marvellous shows the highest average yield among the three varieties.
...
The model suggests that each additional unit of nitrogen increases yield by approximately **`r round(coef(model)[2], 1)`** bushels/acre.
